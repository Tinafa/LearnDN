...
public class UISprite : UIBasicSprite
{
    ...
    public void SetAtlas(string path)
    {
        if (atlasPath != path)
        {
            ...
            {
                GameObject obj = XResourceLoaderMgr.singleton.CreateFromPrefab(path) as GameObject;
                atlas = obj == null ? null : obj.GetComponent<UIAtlas>();
            }

            DestroyAtlas(oldAtlas);
            atlasPath = path;
        }
    }
    private void DestroyAtlas(UIAtlas a)
    {
        if (a != null)
        {
            XResourceLoaderMgr.singleton.UnSafeDestroyShareResource(atlasPath, ".prefab", a.gameObject);
        }
    }
}
