...
public class UIInput : MonoBehaviour
{
    ...
    /// <summary>
    /// Event delegates triggered when get any valid event.
    /// </summary>
    ...
    public KeyCode recentKey = KeyCode.None;
	...
	public virtual bool ProcessEvent (Event ev)
	{
		...
		switch (ev.keyCode)
		{
			...
			{
				ev.Use();
                ExecuteOnKeyTriggered(ev.keyCode);
    ...
    {
        if (current == null && EventDelegate.IsValid(onKeyTriggered))
        {
            current = this;
            this.recentKey = key;
            EventDelegate.Execute(onKeyTriggered);
