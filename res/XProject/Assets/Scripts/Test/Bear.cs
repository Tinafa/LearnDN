...
public class Bear : MonoBehaviour
{
	...
	void FixedUpdate () {
        ...
        if (h * _rid2d.velocity.x < maxSpeed)
            _rid2d.AddForce(Vector2.right * h * moveForce);

        if (Mathf.Abs(_rid2d.velocity.x) > maxSpeed)
            _rid2d.velocity = new Vector2(Mathf.Sign(_rid2d.velocity.x) * maxSpeed, GetComponent<Rigidbody2D>().velocity.y);


        if (v > 0 && IsCanClimbUp())
        {
            _rid2d.AddForce(Vector2.up * v * moveForce);
        }
        if(v < 0 && IsCanClimbDown())
        {
            _rid2d.AddForce(Vector2.down * v * moveForce);
        }
